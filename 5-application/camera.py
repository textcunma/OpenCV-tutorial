...
class Segment:
    ...
    def __call__(self, image):
        blob = cv2.dnn.blobFromImage(
            image, 1 / 255.0, (1024, 512), swapRB=True, crop=False
        )
        ...
        colormask = cv2.resize(
            colormask, (image.shape[1], image.shape[0]), interpolation=cv2.INTER_NEAREST
        )
...
class ObjectDetection:
    def __init__(self):
        ...
        modelConfiguration = (
            "../Yolo-Fastest-opencv-dnn/Yolo-Fastest-voc/yolo-fastest-xl.cfg"
        )
        modelWeights = (
            "../Yolo-Fastest-opencv-dnn/Yolo-Fastest-voc/yolo-fastest-xl.weights"
        )
    ...
    def __call__(self, img):
        blob = cv2.dnn.blobFromImage(
            img, 1 / 255.0, (320, 320), swapRB=True, crop=False
        )
        self.net.setInput(blob)
    ...
    def postprocess(self, img, outs):
        ...
        for i in indices:
            ...
            height = box[3]
            self.drawPred(
                img, classIds[i], confidences[i], left, top, left + width, top + height
            )
