...
def drawPred(classId, conf, left, top, right, bottom):
    cv2.rectangle(
        img, (left, top), (right, bottom), (0, 0, 255), thickness=4
    )  # バウンディングボックス描画
...
def postprocess(img, outs):
    ...
    for out in outs:
        for detection in out:
            ...
            classId = np.argmax(
                scores
            )  # 各種物体クラスの中で信頼度スコアが最大要素の添え字(ID)を取得  # ex) classId=0
            confidence = scores[
                classId
            ]  # 信頼度スコアを取得                                        # ex) confidence=0.95
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image", type=str, default="./img/face.png", help="入力画像パス指定"
    )  ##############################
