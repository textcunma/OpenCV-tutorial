...
matrix=cv2.getRotationMatrix2D((centerX,centerY), degree , ratio)   # 回転、拡大縮小に関する変換行列生成(中心座標,回転角度,拡大縮小比率)
...
trans_matrix = np.float32([[1, 0, centerX], [0, 1, centerY],[0,0,1]])       # 平行移動行列(原点へ移動)
rot_matrix=np.float32([[cos, -sin, 0], [sin,cos,0],[0,0,1]])                # 回転行列
re_trans_matrix=np.float32([[1, 0, -centerX], [0, 1, -centerY],[0,0,1]])    # 平行移動行列(元へ戻す)
