...
def draw(img, output, color, args):
    ...
    colormask = cv2.resize(
        colormask, (img.shape[1], img.shape[0]), interpolation=cv2.INTER_NEAREST
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", default="./ENet/enet-cityscapes/enet-model.net", help="モデルパス"
    )
    parser.add_argument(
        "--classes", default="./ENet/enet-cityscapes/enet-classes.txt", help="クラスラベルパス"
    )
    parser.add_argument("--image", default="./img/car.png", help="入力画像")
    parser.add_argument(
        "--colors",
        type=str,
        default="./ENet/enet-cityscapes/enet-colors.txt",
        help="各クラスに対応した色パス",
    )
    parser.add_argument(
        "--colordisplay", action="store_false", help="出力画像を合成かカラーマスク画像か"
    )  # デフォルト：True
